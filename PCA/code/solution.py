...
class PCA():
    ...
    def _do_pca(self):
        ...
        '''
        ### YOUR CODE HERE
        self.centered_matrix = self.X - self.X.mean(axis=0)
        U, S, V = np.linalg.svd(self.centered_matrix)
        Up = U[:,:self.n_components]
        Xp = np.dot(Up.T, self.centered_matrix)
        ### END YOUR CODE
        return Up, Xp
...
class AE(nn.Module):
    ...
    def _network(self):
        ...
        #self.w = nn.Parameter(torch.randn(self.n_features, self.d_hidden_rep))

        # Note: here for the network without weights sharing
        #self.w_encoder = nn.Parameter(torch.randn(self.d_hidden_rep, self.n_features))
        #self.w_decoder = nn.Parameter(torch.randn(self.n_features, self.d_hidden_rep))

        # Note: here for the network with more layers and nonlinear functions
        #self.enc1 = nn.Linear(in_features=256, out_features=128)
        #self.enc2 = nn.Linear(in_features=192, out_features=128)
        ...
        #self.dec3 = nn.Linear(in_features=128, out_features=192)
    ...
    def _forward(self, X):
        ...
        '''
        #sigmoid = nn.ReLU()
        #X = sigmoid(torch.mm(torch.transpose(self.w, 0, 1), X))
        #reconstructed_output = sigmoid(torch.mm(self.w, X))
        # Note: here for the network without weights sharing
        #X = sigmoid(torch.mm(self.w_encoder, X))
        #reconstructed_output = sigmoid(torch.mm(self.w_decoder, X))
        # Note: here for the network with more layers and nonlinear functions
        #X = X.t()
        #X = nn.functional.tanh(self.enc1(X))
        #X = nn.functional.tanh(self.enc2(X))
        #X = nn.functional.tanh(self.enc3(X))
        #X = nn.functional.tanh(self.enc4(X))
        #X = nn.functional.tanh(self.dec1(X))
        #X = nn.functional.tanh(self.dec2(X))
        #X = nn.functional.tanh(self.dec3(X))
        #reconstructed_output = nn.functional.tanh(self.dec4(X))
        #reconstructed_output = reconstructed_output.t()
    ...
    def reconstruction(self, X):
        ...
        with torch.no_grad():
            for i in range(n_samples):
                ...
                curr_X_re = curr_X_re_tensor.detach().numpy()
                curr_X_re_2 = list()
                for i in np.ndarray.tolist(curr_X_re):
                    curr_X_re_2.append(i[0])
                X_re.append(curr_X_re_2)
